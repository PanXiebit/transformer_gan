...
def build_graph(params):
    ...
    tower_grads = []
    g_tower_grads = []
    g_model = gen_and_dis_2.Generator(params, is_train=True, name_scope="Transformer")
    d_model = gen_and_dis_2.Discriminator(params, is_train=True, name_scope="Discriminator")
        ...
        for i in xrange(flags_obj.num_gpus):
            with tf.device('/gpu:%d' % i):
                with tf.name_scope('%s_%d' % (TOWER_NAME, i)) as scope:
                    ...
                    rewards, roll_mean_loss, real_mean_loss = g_model.get_reward(
